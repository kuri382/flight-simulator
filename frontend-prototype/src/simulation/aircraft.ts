...
export class Aircraft {
    ...
    computeDerivative(inputs: Inputs): State {
        ...
        // 風 (Earth Frame)をBody Frameに変換して迎角等計算にも使用可能。ここでは省略。
        // 重力をBody Frameへ変換
        const gravityBody = CoordinateTransform.earthToBody(this.state.orientation, gravityEarth);

        // Body Frameでの合力
        const Fx_total = fm.Fx + gravityBody[0];
        const Fy_total = fm.Fy + gravityBody[1];
        const Fz_total = fm.Fz + gravityBody[2];
